...
export function SettingsProvider({ children }) {
  ...
  const doSync = React.useCallback(async () => {
    ...
    try {
      ...
      let cloudMemos = [];
      if (user) {
        try { cloudMemos = await pocketBaseService.getUserMemos(); } catch {}
      }
  ...
  const pinnedIds = new Set(Array.isArray(pinned) ? pinned.map(m => String(m.id)) : []);
        ...
        console.warn('doSync merge failed', e);
      }

      // 再进行上行同步（upsert settings & memos）
      if (user) { // need auth
        await pocketBaseService.syncUserData();
      }
      ...
      if (tombstones && tombstones.length) {
        ...
        if (user) {
          for (const id of ids) {
            try { await pocketBaseService.deleteMemo(id); } catch {}
          }
        }
        removeDeletedMemoTombstones(ids);
      }
    } catch (e) { // This is the catch for the outer try block
      console.error("Error during doSync:", e); // Log the error
    } finally {
  ...
  useEffect(() => {
    const maybeRestore = async () => {
      try {
        ...
        // Restore from PocketBase
        if (!user) return; // need auth to restore
        await pocketBaseService.restoreUserData();
        try { window.dispatchEvent(new CustomEvent('app:dataChanged', { detail: { part: 'restore.pocketbase' } })); } catch {}
  ...
  let cloudSettings = null;
      ...
      cloudMemos = await pocketBaseService.getUserMemos();
      cloudSettings = await pocketBaseService.getUserSettings();
  ...
  localStorage.setItem('s3Config', JSON.stringify(mergedSettings.s3Config || { enabled: false, endpoint: '', accessKeyId: '', secretAccessKey: '', bucket: '', region: 'auto', publicUrl: '', provider: 'r2' }));
      ...
      // All deletions and upserts now go through pocketBaseService
      if (!user) throw new Error('请先登录');
      for (const id of toDeleteIds) {
        try { await pocketBaseService.deleteMemo(id); } catch {}
      }
      for (const memo of merged) {
        await pocketBaseService.upsertMemo(memo);
      }
      await pocketBaseService.upsertUserSettings(mergedSettings);
  ...
  const restoreFromPocketBase = async () => {
    if (!user) {
      throw new Error('请先登录');
    }

    try {
      const result = await pocketBaseService.restoreUserData();
      return result;
    } catch (error) {
      console.error('从PocketBase恢复失败:', error);
